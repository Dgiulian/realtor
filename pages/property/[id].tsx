...
import { Hit, PropertyDetails } from '../../types'
import { baseUrl, fetchApi } from '../../utils/fetchApi';
import { GetServerSidePropsContext } from 'next';
import { ParsedUrlQuery } from 'querystring';
import ImageScrollbar from '@components/image-scrolbar';
...
const PropertyDetailsPage = ({ propertyDetails }: Props) => {
  ...
  return (
    <Box maxWidth="1000px" margin="auto" p="4">
      {photos && photos.length > 0 ? <ImageScrollbar data={photos} /> : null}
      <Box w='full' p='6'>
...
export async function getServerSideProps
  ({ params }: GetServerSidePropsContext<Query>) {
  const { id = '' } = params || {};
